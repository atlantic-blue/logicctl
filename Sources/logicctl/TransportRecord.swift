...
extension TransportCommand {
  ...
  /// reported success would say Logic is recording on every Mac where the message arrives nowhere:
  /// the port is missing, or Logic takes no Machine Control input. An agent that read that answer
  /// would play a part into a take that never started, and the performance is gone. So the command
  /// reads the transport back after the message, and a transport that does not record is a failure
  /// with the time it was given.
  struct Record: ParsableCommand {
    static let configuration = CommandConfiguration(
      ...
      discussion: """
        ...
        transport as Logic reads it back, so a transport that never started recording is \
        `timeout` and never a report of a take.
  ...
  static func answer(
    ...
    do {
      ...
      return printer.write(
        Envelope.failure(
          TransportCommand.Record.failure(of: error),
...
/// The take the run records: the Machine Control message, then the transport of Logic.
///
...
private struct StartRecording: LogicCommand {
  ...
  /// Sends the message, waits until Logic reads as recording, and answers the transport it reads.
  ///
  /// The wait reads the condition before it sleeps, so a transport that already records costs the
  /// command nothing. A transport that never starts recording ends at the limit with `timeout` and
